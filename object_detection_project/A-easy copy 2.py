...
# --------------------------------------------------------
# Pre-calcolo istogrammi dei modelli (a colori, a risoluzione originale)
# --------------------------------------------------------
model_histograms = {}
...
def detect_product_in_scene(img_query, img_train):
    ...
    # 6) Lowe ratio test (qui 0.4, più severo del 0.7 del lab)
    good = []
    for m, n in matches:
    ...
    if good_matches > MIN_MATCH_COUNT:
        ...
        # 9) (solo per debug visuale, non lo usiamo dopo)
        ...
        # 10) Estraggo bounding box axis-aligned
...
for scene_name in scene_names:
    ...
    if max(h_s, w_s) > MAX_SIDE:
        scale_s = MAX_SIDE / max(h_s, w_s)
        img_scene = cv2.resize(img_scene, None,
                               fx=scale_s, fy=scale_s,
                               interpolation=cv2.INTER_AREA)
        img_scene_color = cv2.resize(img_scene_color, None,
                                     fx=scale_s, fy=scale_s,
                                     interpolation=cv2.INTER_AREA)
    # (non usiamo più scale_s dopo, lavoriamo tutto nello spazio ridotto)
    ...
    for pid in product_ids:
        ...
        if max(h_m, w_m) > MAX_SIDE:
            scale_m = MAX_SIDE / max(h_m, w_m)
            img_model = cv2.resize(img_model, None,
                                   fx=scale_m, fy=scale_m,
                                   interpolation=cv2.INTER_AREA)
